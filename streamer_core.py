...
"-threads",
    ...
    def _advance_track_unlocked(self) -> None:
        """
        Move to the next track in playlist and restart ONLY the audio decoder.
        Encoder (RTMP) stays up if already running so the stream is seamless.
        Caller must hold self.lock.
        """
        if not self.playlist:
            self.status = "stopped"
        ...
        # If encoder is not running we need to start the whole pipeline
        if self.encoder_proc is None or self.encoder_proc.poll() is not None:
            self._start_pipeline_unlocked(0.0)
        else:
            # Only restart audio decoder for the new track
